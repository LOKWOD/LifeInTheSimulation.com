...
const today = [
  "essays/the-default-is-a-decision-someone-else-made.html",
  "guides/personal-threat-modeling-for-ordinary-people.html",
  "guides/paper-planning-systems-for-focus.html"
];
const assets = [
  "/assets/visuals/defaults-choice-editorial.webp",
  "/assets/visuals/personal-threat-model-editorial.webp",
  "/assets/visuals/paper-planning-editorial.webp"
];
...
if (!/>31<\/span> transmissions available/.test(essaysArchive)) fail("essays archive: stale result count");
if (!/>31<\/span> protocols available/.test(guidesArchive)) fail("field guides archive: stale result count");
if (!/>31<\/dt><dd>Essays/.test(home) || !/>31<\/dt><dd>Field guides/.test(home)) fail("homepage: stale library counts");
for (const rel of today) {
...
for (const host of ["science.org", "nber.org", "cambridge.org", "pnas.org"]) if (!readFileSync(join(root, today[0]), "utf8").includes(host)) fail(`${today[0]}: missing authoritative ${host} source`);
for (const host of ["ssd.eff.org", "nist.gov", "cisa.gov"]) if (!readFileSync(join(root, today[1]), "utf8").includes(host)) fail(`${today[1]}: missing official ${host} source`);
for (const host of ["timeblockplanner.com", "bulletjournal.com", "leuchtturm1917.us"]) if (!readFileSync(join(root, today[2]), "utf8").includes(host)) fail(`${today[2]}: missing official ${host} source`);
...
if (!/<lastBuildDate>Thu, 03 Sep 2026/.test(feed)) fail("feed: stale lastBuildDate");
