...
import { pages } from "./content-2026-09-03.mjs";

const root = resolve(process.argv[2] || ".");
const siteUrl = "https://lifeinthesimulation.com";
const published = "2026-09-03";
const publishedHuman = "September 3, 2026";
const pubDate = "Thu, 03 Sep 2026 13:30:00 GMT";
...
const [essay, threatGuide, plannerGuide] = pages;
...
upsert("essays.html", "DAILY 2026-09-03 ESSAY", `<section class="section section-pad"><div class="section-heading"><div><p class="eyebrow">LATEST TRANSMISSION</p><h2>The path of least resistance was still designed.</h2></div></div><div class="content-grid three">${card(essay)}</div></section>`);
upsert("field-guides.html", "DAILY 2026-09-03 GUIDES", `<section class="section section-pad"><div class="section-heading"><div><p class="eyebrow">LATEST FIELD GUIDES</p><h2>Model the risk. Choose a paper system that solves the real failure.</h2></div></div><div class="content-grid three">${card(threatGuide)}${card(plannerGuide)}</div></section>`);
upsert("index.html", "DAILY 2026-09-03 HOME", `<section class="latest-section section-pad"><div class="section-heading"><div><p class="eyebrow">NEW FIELD NOTES · SEP 3</p><h2>Question the preset. Defend what matters. Give the day a shape.</h2></div><a class="text-link" href="/feed.xml">Follow via RSS <span>→</span></a></div><div class="content-grid three">${card(essay)}${card(threatGuide)}${card(plannerGuide)}</div></section>`, "<section class=\"guide-spotlight\">");

for (const [file, replacements] of Object.entries({
  "index.html": [[">30</dt><dd>Essays", ">31</dt><dd>Essays"], [">29</dt><dd>Field guides", ">31</dt><dd>Field guides"], ["View all 30 essays", "View all 31 essays"]],
  "essays.html": [[">30 / Transmissions", ">31 / Transmissions"], [">30</span> transmissions available", ">31</span> transmissions available"]],
  "field-guides.html": [[">29 / Field guides", ">31 / Field guides"], [">29</span> protocols available", ">31</span> protocols available"]],
...
upsert("essays/friction-is-a-feature.html", "DAILY 2026-09-03 DEFAULTS", `<div class="article-callout"><strong>Inspect the starting state</strong><span>Friction begins before the first click. Use <a href="/essays/the-default-is-a-decision-someone-else-made.html">The Default Is a Decision Someone Else Made</a> to test the preselected path.</span></div>`, "</article>");
upsert("guides/personal-data-minimization.html", "DAILY 2026-09-03 THREAT", `<div class="article-callout"><strong>Model the risk before adding tools</strong><span>Use <a href="/guides/personal-threat-modeling-for-ordinary-people.html">Personal Threat Modeling for Ordinary People</a> to identify the smallest control that changes a plausible failure.</span></div>`, "</article>");
upsert("guides/deep-work-field-manual.html", "DAILY 2026-09-03 PAPER", `<div class="article-callout"><strong>Give the block a physical plan</strong><span>Compare time-blocking, Bullet Journal and index cards in <a href="/guides/paper-planning-systems-for-focus.html">Paper Planning Systems for Focus</a>.</span></div>`, "</article>");

let feed = readFileSync(join(root, "feed.xml"), "utf8").replace(/<lastBuildDate>[^<]+<\/lastBuildDate>/, `<lastBuildDate>${pubDate}</lastBuildDate>`).replace(/<!-- DAILY 2026-09-03 FEED START -->[\s\S]*?<!-- DAILY 2026-09-03 FEED END -->\n?/g, "");
...
const feedBlock = `<!-- DAILY 2026-09-03 FEED START -->\n${feedItems}\n<!-- DAILY 2026-09-03 FEED END -->\n`;
feed = feed.includes("<!-- DAILY 2026-09-02 FEED START -->") ? feed.replace("<!-- DAILY 2026-09-02 FEED START -->", `${feedBlock}    <!-- DAILY 2026-09-02 FEED START -->`) : feed.replace(/\s*<item>/, `\n${feedBlock}<item>`);
...
sitemap = sitemap.replace(/\s*<!-- DAILY 2026-09-03 SITEMAP START -->[\s\S]*?<!-- DAILY 2026-09-03 SITEMAP END -->\s*/g, "");
...
sitemap = sitemap.replace("</urlset>", `\n<!-- DAILY 2026-09-03 SITEMAP START -->\n${urls}\n<!-- DAILY 2026-09-03 SITEMAP END -->\n</urlset>`);
writeFileSync(join(root, "sitemap.xml"), sitemap);

let llms = readFileSync(join(root, "llms.txt"), "utf8").replace(/\n?## Daily publication 2026-09-03[\s\S]*?(?=\n## |$)/g, "");
llms += `\n\n## Daily publication 2026-09-03\n${pages.map((page) => `- ${page.title}: ${siteUrl}/${page.path}`).join("\n")}\n`;
