...
import { pages } from "./content-2026-09-19.mjs";

const root = resolve(process.argv[2] || ".");
const siteUrl = "https://lifeinthesimulation.com";
const published = "2026-09-19";
const publishedHuman = "September 19, 2026";
const pubDate = "Sat, 19 Sep 2026 13:55:00 GMT";
...
const [essay, inferenceGuide, cableGuide] = pages;
...
upsert("essays.html", "DAILY 2026-09-19 ESSAY", `<section class="section section-pad"><div class="section-heading"><div><p class="eyebrow">LATEST TRANSMISSION</p><h2>The image is a measurement pipeline, not a window into thought.</h2></div></div><div class="content-grid three">${card(essay)}</div></section>`);
upsert("field-guides.html", "DAILY 2026-09-19 GUIDES", `<section class="section section-pad"><div class="section-heading"><div><p class="eyebrow">LATEST FIELD GUIDES</p><h2>Expose the inference. Buy the cable by the job.</h2></div></div><div class="content-grid three">${card(inferenceGuide)}${card(cableGuide)}</div></section>`);
upsert("index.html", "DAILY 2026-09-19 HOME", `<section class="latest-section section-pad"><div class="section-heading"><div><p class="eyebrow">NEW FIELD NOTES · SEP 19</p><h2>Read the scan carefully. Inspect the bridge. Specify the cable.</h2></div><a class="text-link" href="/feed.xml">Follow via RSS <span>→</span></a></div><div class="content-grid three">${card(essay)}${card(inferenceGuide)}${card(cableGuide)}</div></section>`, "<section class=\"guide-spotlight\">");

for (const [file, replacements] of Object.entries({
  "index.html": [[">43</dt><dd>Essays", ">44</dt><dd>Essays"], [">55</dt><dd>Field guides", ">57</dt><dd>Field guides"], ["View all 43 essays", "View all 44 essays"]],
  "essays.html": [[">43 / Transmissions", ">44 / Transmissions"], [">43</span> transmissions available", ">44</span> transmissions available"]],
  "field-guides.html": [[">55 / Field guides", ">57 / Field guides"], [">55</span> protocols available", ">57</span> protocols available"]],
})) {
...
upsert("essays/observer-effect-does-not-mean-consciousness-creates-reality.html", "DAILY 2026-09-19 BRAIN SCAN", `<div class="article-callout"><strong>A measurement is not the thing measured</strong><span>Use <a href="/essays/brain-scan-is-not-a-thought.html">the brain-imaging reading card</a> to keep signals, models and experience in their proper layers.</span></div>`, "</article>");
upsert("guides/how-to-read-a-scientific-paper.html", "DAILY 2026-09-19 INFERENCE", `<div class="article-callout"><strong>Inspect the bridge after reading the methods</strong><span>Use the <a href="/guides/how-to-separate-claim-evidence-inference.html">claim–evidence–inference worksheet</a> to expose assumptions and rival explanations.</span></div>`, "</article>");
upsert("guides/how-to-verify-an-ai-citation.html", "DAILY 2026-09-19 INFERENCE", `<div class="article-callout"><strong>A real citation can still support the wrong conclusion</strong><span>Separate <a href="/guides/how-to-separate-claim-evidence-inference.html">the claim, the source evidence and the inference</a> before carrying an AI answer into real work.</span></div>`, "</article>");
upsert("guides/usb-c-hub-vs-thunderbolt-dock-vs-monitor-dock.html", "DAILY 2026-09-19 USB CABLE", `<div class="article-callout"><strong>The dock cannot outrun the cable</strong><span>Match <a href="/guides/usb-c-cables-charge-data-video.html">USB-C cable power, data and video capability</a> to the whole connection before blaming the dock.</span></div>`, "</article>");
upsert("guides/kvm-switch-vs-usb-switch-vs-dock.html", "DAILY 2026-09-19 USB CABLE", `<div class="article-callout"><strong>Switching hardware still depends on the link</strong><span>Use the <a href="/guides/usb-c-cables-charge-data-video.html">USB-C cable capability guide</a> to verify power, data rate and display support separately.</span></div>`, "</article>");

let feed = readFileSync(join(root, "feed.xml"), "utf8").replace(/<lastBuildDate>[^<]+<\/lastBuildDate>/, `<lastBuildDate>${pubDate}</lastBuildDate>`).replace(/<!-- DAILY 2026-09-19 FEED START -->[\s\S]*?<!-- DAILY 2026-09-19 FEED END -->\n?/g, "");
...
const feedBlock = `<!-- DAILY 2026-09-19 FEED START -->\n${feedItems}\n<!-- DAILY 2026-09-19 FEED END -->\n`;
feed = feed.includes("<!-- DAILY 2026-09-18 FEED START -->") ? feed.replace("<!-- DAILY 2026-09-18 FEED START -->", `${feedBlock}    <!-- DAILY 2026-09-18 FEED START -->`) : feed.replace(/\s*<item>/, `\n${feedBlock}<item>`);
