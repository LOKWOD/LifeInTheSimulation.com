...
import { pages } from "./content-2026-09-22.mjs";

const root = resolve(process.argv[2] || ".");
const siteUrl = "https://lifeinthesimulation.com";
const published = "2026-09-22";
const publishedHuman = "September 22, 2026";
const pubDate = "Tue, 22 Sep 2026 13:50:00 GMT";
...
const [essay, assumptionGuide, trackerGuide] = pages;
...
upsert("essays.html", "DAILY 2026-09-22 ESSAY", `<section class="section section-pad"><div class="section-heading"><div><p class="eyebrow">LATEST TRANSMISSION</p><h2>The waiting time is part of what the system does.</h2></div></div><div class="content-grid three">${card(essay)}</div></section>`);
upsert("field-guides.html", "DAILY 2026-09-22 GUIDES", `<section class="section section-pad"><div class="section-heading"><div><p class="eyebrow">LATEST FIELD GUIDES</p><h2>Expose the premise. Choose the finding network.</h2></div></div><div class="content-grid three">${card(assumptionGuide)}${card(trackerGuide)}</div></section>`);
upsert("index.html", "DAILY 2026-09-22 HOME", `<section class="latest-section section-pad"><div class="section-heading"><div><p class="eyebrow">NEW FIELD NOTES · SEP 22</p><h2>Read the delay. Test the premise. Find the object.</h2></div><a class="text-link" href="/feed.xml">Follow via RSS <span>→</span></a></div><div class="content-grid three">${card(essay)}${card(assumptionGuide)}${card(trackerGuide)}</div></section>`, "<section class=\"guide-spotlight\">");

for (const [file, replacements] of Object.entries({
  "index.html": [[/>\d+<\/dt><dd>Essays/, ">47</dt><dd>Essays"], [/>\d+<\/dt><dd>Field guides/, ">63</dt><dd>Field guides"], [/View all \d+ essays/, "View all 47 essays"]],
  "essays.html": [[/>\d+ \/ Transmissions/, ">47 / Transmissions"], [/>\d+<\/span> transmissions available/, ">47</span> transmissions available"]],
  "field-guides.html": [[/>\d+ \/ Field guides/, ">63 / Field guides"], [/>\d+<\/span> protocols available/, ">63</span> protocols available"]],
})) {
...
upsert("essays/a-system-is-not-resilient-because-it-has-never-failed.html", "DAILY 2026-09-22 DELAY", `<div class="article-callout"><strong>Recovery time belongs inside resilience</strong><span>Use <a href="/essays/system-includes-the-waiting-time.html">the end-to-end delay audit</a> to find queues, feedback gaps and deadlines hidden by normal operation.</span></div>`, "</article>");
upsert("guides/personal-dependency-map.html", "DAILY 2026-09-22 DELAY", `<div class="article-callout"><strong>A dependency can fail by arriving too late</strong><span>Read <a href="/essays/system-includes-the-waiting-time.html">why the system includes the waiting time</a> before accepting a fallback that misses the decision window.</span></div>`, "</article>");
upsert("guides/decision-journal.html", "DAILY 2026-09-22 ASSUMPTIONS", `<div class="article-callout"><strong>Give decision-bearing assumptions their own review path</strong><span>Build an <a href="/guides/how-to-build-an-assumption-register.html">eight-field assumption register</a> for the premises that can still change the plan.</span></div>`, "</article>");
upsert("guides/how-to-run-a-premortem.html", "DAILY 2026-09-22 ASSUMPTIONS", `<div class="article-callout"><strong>Turn the pre-mortem into maintained evidence</strong><span>Move plan-breaking premises into an <a href="/guides/how-to-build-an-assumption-register.html">assumption register with owners, triggers and fallbacks</a>.</span></div>`, "</article>");
upsert("guides/personal-threat-modeling-for-ordinary-people.html", "DAILY 2026-09-22 TRACKERS", `<div class="article-callout"><strong>An item tracker creates a location-data relationship</strong><span>Compare <a href="/guides/bluetooth-item-trackers-find-my-find-hub-tile.html">Find My, Find Hub, Tile and GPS architectures</a> before choosing the device.</span></div>`, "</article>");
upsert("guides/personal-data-minimization.html", "DAILY 2026-09-22 TRACKERS", `<div class="article-callout"><strong>Minimize location history after the tag is attached</strong><span>Use the <a href="/guides/bluetooth-item-trackers-find-my-find-hub-tile.html">tracker account-and-data check</a> to review sharing, history, recovery and disposal.</span></div>`, "</article>");

let feed = readFileSync(join(root, "feed.xml"), "utf8").replace(/<lastBuildDate>[^<]+<\/lastBuildDate>/, `<lastBuildDate>${pubDate}</lastBuildDate>`).replace(/^[ \t]*<!-- DAILY 2026-09-22 FEED START -->[\s\S]*?<!-- DAILY 2026-09-22 FEED END -->\n?/gm, "");
...
const feedBlock = `<!-- DAILY 2026-09-22 FEED START -->\n${feedItems}\n<!-- DAILY 2026-09-22 FEED END -->\n`;
const previousMarker = feed.includes("<!-- DAILY 2026-09-21 FEED START -->") ? "<!-- DAILY 2026-09-21 FEED START -->" : "<!-- DAILY 2026-09-20 FEED START -->";
