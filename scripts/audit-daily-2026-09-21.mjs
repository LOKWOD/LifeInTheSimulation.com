...
const today = [
  "essays/not-detected-is-not-not-present.html",
  "guides/personal-dependency-map.html",
  "guides/home-security-cameras-local-cloud-hybrid.html"
];
const assets = [
  "/assets/visuals/detection-limits-editorial.webp",
  "/assets/visuals/personal-dependency-map-editorial.webp",
  "/assets/visuals/home-security-camera-storage-editorial.webp"
];
...
for (const host of ["itl.nist.gov", "epa.gov", "uncertainty.nist.gov"]) if (!readFileSync(join(root, today[0]), "utf8").includes(host)) fail(`${today[0]}: missing authoritative ${host} source`);
for (const host of ["csrc.nist.gov", "ready.gov"]) if (!readFileSync(join(root, today[1]), "utf8").includes(host)) fail(`${today[1]}: missing authoritative ${host} source`);
for (const host of ["csrc.nist.gov", "fcc.gov", "onvif.org"]) if (!readFileSync(join(root, today[2]), "utf8").includes(host)) fail(`${today[2]}: missing authoritative ${host} source`);
...
if (!/<lastBuildDate>Mon, 21 Sep 2026/.test(feed)) fail("feed: stale lastBuildDate");
...
for (const [hub, rel] of [["topics/simulation-theory.html", today[0]], ["topics/privacy-security.html", today[1]], ["topics/privacy-security.html", today[2]]]) if (!readFileSync(join(root, hub), "utf8").includes(`/${rel}`)) fail(`${hub}: missing ${rel}`);
for (const [source, target] of [["essays/uncertainty-is-information-not-failure.html", today[0]], ["guides/how-to-run-a-one-person-experiment.html", today[0]], ["essays/a-system-is-not-resilient-because-it-has-never-failed.html", today[1]], ["guides/personal-digital-exit-plan.html", today[1]], ["guides/personal-threat-modeling-for-ordinary-people.html", today[2]], ["guides/home-wifi-router-vs-mesh-vs-access-points.html", today[2]]]) if (!readFileSync(join(root, source), "utf8").includes(`/${target}`)) fail(`${source}: missing reciprocal link to ${target}`);
