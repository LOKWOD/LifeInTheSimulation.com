...
const today = [
  "essays/digital-twin-is-not-a-second-reality.html",
  "guides/private-dns-quad9-cloudflare-nextdns.html",
  "guides/minimalist-phones-light-phone-punkt-locked-down-smartphone.html"
];
const assets = [
  "/assets/visuals/digital-twin-editorial.webp",
  "/assets/visuals/private-dns-editorial.webp",
  "/assets/visuals/minimalist-phone-editorial.webp"
];
...
if (!/>30<\/span> transmissions available/.test(essaysArchive)) fail("essays archive: stale result count");
if (!/>29<\/span> protocols available/.test(guidesArchive)) fail("field guides archive: stale result count");
if (!/>30<\/dt><dd>Essays/.test(home) || !/>29<\/dt><dd>Field guides/.test(home)) fail("homepage: stale library counts");
for (const rel of today) {
...
for (const host of ["nist.gov"]) if (!readFileSync(join(root, today[0]), "utf8").includes(host)) fail(`${today[0]}: missing authoritative ${host} source`);
for (const host of ["quad9.net", "developers.cloudflare.com", "nextdns.io"]) if (!readFileSync(join(root, today[1]), "utf8").includes(host)) fail(`${today[1]}: missing official ${host} source`);
for (const host of ["thelightphone.com", "punkt.ch", "support.apple.com", "support.google.com"]) if (!readFileSync(join(root, today[2]), "utf8").includes(host)) fail(`${today[2]}: missing official ${host} source`);

if (!/<lastBuildDate>Wed, 02 Sep 2026/.test(feed)) fail("feed: stale lastBuildDate");
