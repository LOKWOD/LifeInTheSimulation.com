...
import { pages } from "./content-2026-09-13.mjs";

const root = resolve(process.argv[2] || ".");
const siteUrl = "https://lifeinthesimulation.com";
const published = "2026-09-13";
const publishedHuman = "September 13, 2026";
const pubDate = "Sun, 13 Sep 2026 13:43:00 GMT";
...
const [essay, ledgerGuide, dockGuide] = pages;
...
upsert("essays.html", "DAILY 2026-09-13 ESSAY", `<section class="section section-pad"><div class="section-heading"><div><p class="eyebrow">LATEST TRANSMISSION</p><h2>The detector is physical. The interpretation remains open.</h2></div></div><div class="content-grid three">${card(essay)}</div></section>`);
upsert("field-guides.html", "DAILY 2026-09-13 GUIDES", `<section class="section section-pad"><div class="section-heading"><div><p class="eyebrow">LATEST FIELD GUIDES</p><h2>Trace the claim. Verify the connection.</h2></div></div><div class="content-grid three">${card(ledgerGuide)}${card(dockGuide)}</div></section>`);
upsert("index.html", "DAILY 2026-09-13 HOME", `<section class="latest-section section-pad"><div class="section-heading"><div><p class="eyebrow">NEW FIELD NOTES · SEP 13</p><h2>Separate observation from awareness. Make evidence inspectable. Buy the verified port path.</h2></div><a class="text-link" href="/feed.xml">Follow via RSS <span>→</span></a></div><div class="content-grid three">${card(essay)}${card(ledgerGuide)}${card(dockGuide)}</div></section>`, "<section class=\"guide-spotlight\">");

for (const [file, replacements] of Object.entries({
  "index.html": [[">38</dt><dd>Essays", ">39</dt><dd>Essays"], [">45</dt><dd>Field guides", ">47</dt><dd>Field guides"], ["View all 38 essays", "View all 39 essays"]],
  "essays.html": [[">38 / Transmissions", ">39 / Transmissions"], [">38</span> transmissions available", ">39</span> transmissions available"]],
  "field-guides.html": [[">45 / Field guides", ">47 / Field guides"], [">45</span> protocols available", ">47</span> protocols available"]],
})) {
...
upsert("essays/dreams-are-not-evidence-of-simulation.html", "DAILY 2026-09-13 OBSERVER", `<div class="article-callout"><strong>Keep observation physical</strong><span>Read <a href="/essays/observer-effect-does-not-mean-consciousness-creates-reality.html">The Observer Effect Does Not Mean Consciousness Creates Reality</a> to separate detector interactions from claims about awareness.</span></div>`, "</article>");
upsert("guides/how-to-verify-an-ai-citation.html", "DAILY 2026-09-13 LEDGER", `<div class="article-callout"><strong>Carry verification into the workflow</strong><span>Use <a href="/guides/how-to-build-an-ai-evidence-ledger.html">an AI evidence ledger</a> to connect consequential claims to locators, status, owners and refresh triggers.</span></div>`, "</article>");
upsert("guides/home-wifi-router-vs-mesh-vs-access-points.html", "DAILY 2026-09-13 DOCK", `<div class="article-callout"><strong>Verify the wired desk too</strong><span>Use <a href="/guides/usb-c-hub-vs-thunderbolt-dock-vs-monitor-dock.html">USB-C Hubs and Docks Without the Hype</a> to match the host, displays, power and cable before buying.</span></div>`, "</article>");

let feed = readFileSync(join(root, "feed.xml"), "utf8").replace(/<lastBuildDate>[^<]+<\/lastBuildDate>/, `<lastBuildDate>${pubDate}</lastBuildDate>`).replace(/<!-- DAILY 2026-09-13 FEED START -->[\s\S]*?<!-- DAILY 2026-09-13 FEED END -->\n?/g, "");
...
const feedBlock = `<!-- DAILY 2026-09-13 FEED START -->\n${feedItems}\n<!-- DAILY 2026-09-13 FEED END -->\n`;
feed = feed.includes("<!-- DAILY 2026-09-11 FEED START -->") ? feed.replace("<!-- DAILY 2026-09-11 FEED START -->", `${feedBlock}    <!-- DAILY 2026-09-11 FEED START -->`) : feed.replace(/\s*<item>/, `\n${feedBlock}<item>`);
