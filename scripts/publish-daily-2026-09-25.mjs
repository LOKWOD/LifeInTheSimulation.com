...
import { pages } from "./content-2026-09-25.mjs";

const root = resolve(process.argv[2] || ".");
const siteUrl = "https://lifeinthesimulation.com";
const published = "2026-09-25";
const publishedHuman = "September 25, 2026";
const pubDate = "Fri, 25 Sep 2026 13:37:00 GMT";
...
const [essay, runbookGuide, frameGuide] = pages;
...
upsert("essays.html", "DAILY 2026-09-25 ESSAY", `<section class="section section-pad"><div class="section-heading"><div><p class="eyebrow">LATEST TRANSMISSION</p><h2>Selection is not an explanation of experience.</h2></div></div><div class="content-grid three">${card(essay)}</div></section>`);
upsert("field-guides.html", "DAILY 2026-09-25 GUIDES", `<section class="section section-pad"><div class="section-heading"><div><p class="eyebrow">LATEST FIELD GUIDES</p><h2>Preserve the next action. Choose the photo path.</h2></div></div><div class="content-grid three">${card(runbookGuide)}${card(frameGuide)}</div></section>`);
upsert("index.html", "DAILY 2026-09-25 HOME", `<section class="latest-section section-pad"><div class="section-heading"><div><p class="eyebrow">NEW FIELD NOTES · SEP 25</p><h2>Separate the experience. Write the procedure. Keep the archive.</h2></div><a class="text-link" href="/feed.xml">Follow via RSS <span>→</span></a></div><div class="content-grid three">${card(essay)}${card(runbookGuide)}${card(frameGuide)}</div></section>`, "<section class=\"guide-spotlight\">");

for (const [file, replacements] of Object.entries({
  "index.html": [[/>\d+<\/dt><dd>Essays/, ">48</dt><dd>Essays"], [/>\d+<\/dt><dd>Field guides/, ">65</dt><dd>Field guides"], [/View all \d+ essays/, "View all 48 essays"]],
  "essays.html": [[/>\d+ \/ Transmissions/, ">48 / Transmissions"], [/>\d+<\/span> transmissions available/, ">48</span> transmissions available"]],
  "field-guides.html": [[/>\d+ \/ Field guides/, ">65 / Field guides"], [/>\d+<\/span> protocols available/, ">65</span> protocols available"]],
})) {
...
upsert("essays/consciousness-is-the-weird-part.html", "DAILY 2026-09-25 ATTENTION", `<div class="article-callout"><strong>Selection and experience answer different questions</strong><span>Use the <a href="/essays/attention-is-not-consciousness.html">attention–awareness matrix</a> before treating prioritization or report as consciousness itself.</span></div>`, "</article>");
upsert("essays/intelligence-is-not-consciousness.html", "DAILY 2026-09-25 ATTENTION", `<div class="article-callout"><strong>Computational attention is not evidence of experience</strong><span>Read why <a href="/essays/attention-is-not-consciousness.html">attention and consciousness must remain separate claims</a>.</span></div>`, "</article>");
upsert("guides/personal-dependency-map.html", "DAILY 2026-09-25 RUNBOOK", `<div class="article-callout"><strong>Turn a critical path into an executable procedure</strong><span>Write a <a href="/guides/how-to-write-a-personal-runbook.html">runbook with triggers, stop rules and verification</a> for the dependencies that cannot remain in one person's memory.</span></div>`, "</article>");
upsert("essays/a-system-is-not-resilient-because-it-has-never-failed.html", "DAILY 2026-09-25 RUNBOOK", `<div class="article-callout"><strong>Recovery evidence needs a maintained procedure</strong><span>Build and cold-test a <a href="/guides/how-to-write-a-personal-runbook.html">personal runbook before urgency changes the plan</a>.</span></div>`, "</article>");
upsert("guides/personal-data-minimization.html", "DAILY 2026-09-25 FRAMES", `<div class="article-callout"><strong>A photo frame should not inherit the whole archive</strong><span>Compare <a href="/guides/digital-photo-frames-local-cloud-smart-display.html">local, vendor-cloud and smart-display custody</a> before connecting a library.</span></div>`, "</article>");
upsert("guides/personal-digital-exit-plan.html", "DAILY 2026-09-25 FRAMES", `<div class="article-callout"><strong>Keep the originals outside the display service</strong><span>Use the <a href="/guides/digital-photo-frames-local-cloud-smart-display.html">photo-frame fit card</a> to verify export, deletion and offline behavior before gifting.</span></div>`, "</article>");

let feed = readFileSync(join(root, "feed.xml"), "utf8").replace(/<lastBuildDate>[^<]+<\/lastBuildDate>/, `<lastBuildDate>${pubDate}</lastBuildDate>`).replace(/^[ \t]*<!-- DAILY 2026-09-25 FEED START -->[\s\S]*?<!-- DAILY 2026-09-25 FEED END -->\n?/gm, "");
...
const feedBlock = `<!-- DAILY 2026-09-25 FEED START -->\n${feedItems}\n<!-- DAILY 2026-09-25 FEED END -->\n`;
const previousMarker = "<!-- DAILY 2026-09-22 FEED START -->";
