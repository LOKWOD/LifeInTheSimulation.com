...
import { pages } from "./content-2026-09-04.mjs";

const root = resolve(process.argv[2] || ".");
const siteUrl = "https://lifeinthesimulation.com";
const published = "2026-09-04";
const publishedHuman = "September 4, 2026";
const pubDate = "Fri, 04 Sep 2026 13:30:00 GMT";
...
for (const [file, replacements] of Object.entries({
  "index.html": [[">31</dt><dd>Essays", ">32</dt><dd>Essays"], [">31</dt><dd>Field guides", ">33</dt><dd>Field guides"], ["View all 31 essays", "View all 32 essays"]],
  "essays.html": [[">31 / Transmissions", ">32 / Transmissions"], [">31</span> transmissions available", ">32</span> transmissions available"]],
  "field-guides.html": [[">31 / Field guides", ">33 / Field guides"], [">31</span> protocols available", ">33</span> protocols available"]],
...
upsert("essays/confidence-is-not-calibration.html", "DAILY 2026-09-04 UNCERTAINTY", `<div class="article-callout"><strong>A range is part of the answer</strong><span>Read <a href="/essays/uncertainty-is-information-not-failure.html">Uncertainty Is Information, Not Failure</a> for a practical language of limits, thresholds and revision.</span></div>`, "</article>");
upsert("essays/search-before-wonder.html", "DAILY 2026-09-04 PAPERS", `<div class="article-callout"><strong>Keep the question when the paper arrives</strong><span>Use <a href="/guides/how-to-read-a-scientific-paper.html">How to Read a Scientific Paper Without Borrowing Its Confidence</a> to inspect design, effect, bias and applicability.</span></div>`, "</article>");
upsert("guides/personal-data-minimization.html", "DAILY 2026-09-04 SCANNERS", `<div class="article-callout"><strong>Digitize deliberately</strong><span>Compare batch machines with the no-buy phone option in <a href="/guides/document-scanners-without-the-hype.html">Document Scanners Without the Hype</a>.</span></div>`, "</article>");

let feed = readFileSync(join(root, "feed.xml"), "utf8").replace(/<lastBuildDate>[^<]+<\/lastBuildDate>/, `<lastBuildDate>${pubDate}</lastBuildDate>`).replace(/<!-- DAILY 2026-09-04 FEED START -->[\s\S]*?<!-- DAILY 2026-09-04 FEED END -->\n?/g, "");
...
const feedBlock = `<!-- DAILY 2026-09-04 FEED START -->\n${feedItems}\n<!-- DAILY 2026-09-04 FEED END -->\n`;
feed = feed.includes("<!-- DAILY 2026-09-03 FEED START -->") ? feed.replace("<!-- DAILY 2026-09-03 FEED START -->", `${feedBlock}    <!-- DAILY 2026-09-03 FEED START -->`) : feed.replace(/\s*<item>/, `\n${feedBlock}<item>`);
...
sitemap = sitemap.replace(/\s*<!-- DAILY 2026-09-04 SITEMAP START -->[\s\S]*?<!-- DAILY 2026-09-04 SITEMAP END -->\s*/g, "");
...
sitemap = sitemap.replace("</urlset>", `\n<!-- DAILY 2026-09-04 SITEMAP START -->\n${urls}\n<!-- DAILY 2026-09-04 SITEMAP END -->\n</urlset>`);
writeFileSync(join(root, "sitemap.xml"), sitemap);

let llms = readFileSync(join(root, "llms.txt"), "utf8").replace(/\n?## Daily publication 2026-09-04[\s\S]*?(?=\n## |$)/g, "");
llms += `\n\n## Daily publication 2026-09-04\n${pages.map((page) => `- ${page.title}: ${siteUrl}/${page.path}`).join("\n")}\n`;
