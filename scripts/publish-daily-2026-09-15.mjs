...
import { pages } from "./content-2026-09-15.mjs";

const root = resolve(process.argv[2] || ".");
const siteUrl = "https://lifeinthesimulation.com";
const published = "2026-09-15";
const publishedHuman = "September 15, 2026";
const pubDate = "Tue, 15 Sep 2026 13:24:00 GMT";
...
const [essay, workflowGuide, smartHomeGuide] = pages;
...
upsert("essays.html", "DAILY 2026-09-15 ESSAY", `<section class="section section-pad"><div class="section-heading"><div><p class="eyebrow">LATEST TRANSMISSION</p><h2>A forecast is conditional evidence—not a future fact.</h2></div></div><div class="content-grid three">${card(essay)}</div></section>`);
upsert("field-guides.html", "DAILY 2026-09-15 GUIDES", `<section class="section section-pad"><div class="section-heading"><div><p class="eyebrow">LATEST FIELD GUIDES</p><h2>Audit the workflow. Buy the missing role.</h2></div></div><div class="content-grid three">${card(workflowGuide)}${card(smartHomeGuide)}</div></section>`);
upsert("index.html", "DAILY 2026-09-15 HOME", `<section class="latest-section section-pad"><div class="section-heading"><div><p class="eyebrow">NEW FIELD NOTES · SEP 15</p><h2>Read forecasts conditionally. Audit AI before scale. Separate the smart-home roles.</h2></div><a class="text-link" href="/feed.xml">Follow via RSS <span>→</span></a></div><div class="content-grid three">${card(essay)}${card(workflowGuide)}${card(smartHomeGuide)}</div></section>`, "<section class=\"guide-spotlight\">");

for (const [file, replacements] of Object.entries({
  "index.html": [[">39</dt><dd>Essays", ">40</dt><dd>Essays"], [">47</dt><dd>Field guides", ">49</dt><dd>Field guides"], ["View all 39 essays", "View all 40 essays"]],
  "essays.html": [[">39 / Transmissions", ">40 / Transmissions"], [">39</span> transmissions available", ">40</span> transmissions available"]],
  "field-guides.html": [[">47 / Field guides", ">49 / Field guides"], [">47</span> protocols available", ">49</span> protocols available"]],
})) {
...
upsert("essays/confidence-is-not-calibration.html", "DAILY 2026-09-15 FORECAST", `<div class="article-callout"><strong>Keep the future conditional</strong><span>Read <a href="/essays/forecast-is-not-a-future.html">A Forecast Is Not the Future</a> to separate a point estimate from its assumptions, range, horizon and decision threshold.</span></div>`, "</article>");
upsert("essays/automation-does-not-remove-responsibility.html", "DAILY 2026-09-15 WORKFLOW", `<div class="article-callout"><strong>Audit before permission expands</strong><span>Use <a href="/guides/how-to-audit-an-ai-workflow-before-automation.html">the AI workflow audit</a> to define tests, review gates, logs and rollback before automation can act.</span></div>`, "</article>");
upsert("guides/home-wifi-router-vs-mesh-vs-access-points.html", "DAILY 2026-09-15 SMART HOME", `<div class="article-callout"><strong>Name the smart-home role</strong><span>Use <a href="/guides/smart-home-hubs-matter-controller-thread-border-router-bridge.html">Smart-Home Hubs Without the Hype</a> to distinguish controllers, Thread border routers and bridges before buying.</span></div>`, "</article>");

let feed = readFileSync(join(root, "feed.xml"), "utf8").replace(/<lastBuildDate>[^<]+<\/lastBuildDate>/, `<lastBuildDate>${pubDate}</lastBuildDate>`).replace(/<!-- DAILY 2026-09-15 FEED START -->[\s\S]*?<!-- DAILY 2026-09-15 FEED END -->\n?/g, "");
...
const feedBlock = `<!-- DAILY 2026-09-15 FEED START -->\n${feedItems}\n<!-- DAILY 2026-09-15 FEED END -->\n`;
feed = feed.includes("<!-- DAILY 2026-09-13 FEED START -->") ? feed.replace("<!-- DAILY 2026-09-13 FEED START -->", `${feedBlock}    <!-- DAILY 2026-09-13 FEED START -->`) : feed.replace(/\s*<item>/, `\n${feedBlock}<item>`);
