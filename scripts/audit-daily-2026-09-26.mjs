...
const today = [
  "essays/the-average-is-not-the-experience.html",
  "guides/shared-access-without-shared-passwords.html",
  "guides/portable-monitors-usb-c-hdmi-wireless.html"
];
const assets = [
  "/assets/visuals/average-distribution-editorial.webp",
  "/assets/visuals/shared-access-boundaries-editorial.webp",
  "/assets/visuals/portable-monitor-paths-editorial.webp"
];
...
for (const rel of today) {
  ...
  if (!imageFile || !existsSync(imageFile) || statSync(imageFile).size < 20000) fail(`${rel}: missing or implausibly small editorial asset ${imageTag?.[1] || "(none)"}`);
...
for (const host of ["itl.nist.gov", "plato.stanford.edu"]) if (!readFileSync(join(root, today[0]), "utf8").includes(host)) fail(`${today[0]}: missing authoritative ${host} source`);
for (const host of ["csrc.nist.gov", "cisa.gov", "support.apple.com", "support.google.com"]) if (!readFileSync(join(root, today[1]), "utf8").includes(host)) fail(`${today[1]}: missing authoritative ${host} source`);
for (const host of ["displayport.org", "usb.org", "support.microsoft.com", "support.apple.com"]) if (!readFileSync(join(root, today[2]), "utf8").includes(host)) fail(`${today[2]}: missing authoritative ${host} source`);
...
if (!/<lastBuildDate>Sat, 26 Sep 2026/.test(feed)) fail("feed: stale lastBuildDate");
...
for (const [hub, rel] of [["topics/ai-knowledge.html", today[0]], ["topics/privacy-security.html", today[1]], ["topics/attention-agency.html", today[2]]]) if (!readFileSync(join(root, hub), "utf8").includes(`/${rel}`)) fail(`${hub}: missing ${rel}`);
for (const [source, target] of [["essays/dataset-is-not-the-world.html", today[0]], ["essays/system-includes-the-waiting-time.html", today[0]], ["guides/password-managers-without-the-hype.html", today[1]], ["guides/personal-data-minimization.html", today[1]], ["guides/usb-c-cables-charge-data-video.html", today[2]], ["guides/kvm-switch-vs-usb-switch-vs-dock.html", today[2]]]) if (!readFileSync(join(root, source), "utf8").includes(`/${target}`)) fail(`${source}: missing reciprocal link to ${target}`);
