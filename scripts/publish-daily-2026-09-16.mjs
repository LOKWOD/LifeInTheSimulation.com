...
import { pages } from "./content-2026-09-16.mjs";

const root = resolve(process.argv[2] || ".");
const siteUrl = "https://lifeinthesimulation.com";
const published = "2026-09-16";
const publishedHuman = "September 16, 2026";
const pubDate = "Wed, 16 Sep 2026 13:14:00 GMT";
...
const [essay, stopRuleGuide, networkGuide] = pages;
...
upsert("essays.html", "DAILY 2026-09-16 ESSAY", `<section class="section section-pad"><div class="section-heading"><div><p class="eyebrow">LATEST TRANSMISSION</p><h2>A copy can continue your pattern without settling who survived.</h2></div></div><div class="content-grid three">${card(essay)}</div></section>`);
upsert("field-guides.html", "DAILY 2026-09-16 GUIDES", `<section class="section section-pad"><div class="section-heading"><div><p class="eyebrow">LATEST FIELD GUIDES</p><h2>Define enough. Use the wiring the house can support.</h2></div></div><div class="content-grid three">${card(stopRuleGuide)}${card(networkGuide)}</div></section>`);
upsert("index.html", "DAILY 2026-09-16 HOME", `<section class="latest-section section-pad"><div class="section-heading"><div><p class="eyebrow">NEW FIELD NOTES · SEP 16</p><h2>Separate copies from continuity. Decide what enough means. Choose the honest network path.</h2></div><a class="text-link" href="/feed.xml">Follow via RSS <span>→</span></a></div><div class="content-grid three">${card(essay)}${card(stopRuleGuide)}${card(networkGuide)}</div></section>`, "<section class=\"guide-spotlight\">");

for (const [file, replacements] of Object.entries({
  "index.html": [[">40</dt><dd>Essays", ">41</dt><dd>Essays"], [">49</dt><dd>Field guides", ">51</dt><dd>Field guides"], ["View all 40 essays", "View all 41 essays"]],
  "essays.html": [[">40 / Transmissions", ">41 / Transmissions"], [">40</span> transmissions available", ">41</span> transmissions available"]],
  "field-guides.html": [[">49 / Field guides", ">51 / Field guides"], [">49</span> protocols available", ">51</span> protocols available"]],
})) {
...
upsert("essays/self-model-is-not-a-fake-self.html", "DAILY 2026-09-16 COPY", `<div class="article-callout"><strong>Continuity is a separate claim</strong><span>Read <a href="/essays/mind-upload-copy-personal-identity.html">A Copy of You Is Not Automatically You</a> to separate emulation, consciousness and personal identity.</span></div>`, "</article>");
upsert("guides/decision-journal.html", "DAILY 2026-09-16 STOP RULE", `<div class="article-callout"><strong>Give the search an ending</strong><span>Use <a href="/guides/stop-rule-decide-what-enough-means.html">The Stop Rule</a> to define evidence, budget, guardrails and escalation before work begins.</span></div>`, "</article>");
upsert("guides/home-wifi-router-vs-mesh-vs-access-points.html", "DAILY 2026-09-16 BACKHAUL", `<div class="article-callout"><strong>Choose the path through the building</strong><span>Compare <a href="/guides/home-networking-moca-vs-powerline-vs-mesh.html">MoCA, powerline and wireless mesh</a> when a new Ethernet run is not practical.</span></div>`, "</article>");

let feed = readFileSync(join(root, "feed.xml"), "utf8").replace(/<lastBuildDate>[^<]+<\/lastBuildDate>/, `<lastBuildDate>${pubDate}</lastBuildDate>`).replace(/<!-- DAILY 2026-09-16 FEED START -->[\s\S]*?<!-- DAILY 2026-09-16 FEED END -->\n?/g, "");
...
const feedBlock = `<!-- DAILY 2026-09-16 FEED START -->\n${feedItems}\n<!-- DAILY 2026-09-16 FEED END -->\n`;
feed = feed.includes("<!-- DAILY 2026-09-15 FEED START -->") ? feed.replace("<!-- DAILY 2026-09-15 FEED START -->", `${feedBlock}    <!-- DAILY 2026-09-15 FEED START -->`) : feed.replace(/\s*<item>/, `\n${feedBlock}<item>`);
