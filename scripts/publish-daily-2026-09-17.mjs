...
import { pages } from "./content-2026-09-17.mjs";

const root = resolve(process.argv[2] || ".");
const siteUrl = "https://lifeinthesimulation.com";
const published = "2026-09-17";
const publishedHuman = "September 17, 2026";
const pubDate = "Thu, 17 Sep 2026 13:44:00 GMT";
...
const [essay, learningGuide, deskGuide] = pages;
...
upsert("essays.html", "DAILY 2026-09-17 ESSAY", `<section class="section section-pad"><div class="section-heading"><div><p class="eyebrow">LATEST TRANSMISSION</p><h2>Capability can be extraordinary without settling experience.</h2></div></div><div class="content-grid three">${card(essay)}</div></section>`);
upsert("field-guides.html", "DAILY 2026-09-17 GUIDES", `<section class="section section-pad"><div class="section-heading"><div><p class="eyebrow">LATEST FIELD GUIDES</p><h2>Learn the durable stack. Trace the desk before buying.</h2></div></div><div class="content-grid three">${card(learningGuide)}${card(deskGuide)}</div></section>`);
upsert("index.html", "DAILY 2026-09-17 HOME", `<section class="latest-section section-pad"><div class="section-heading"><div><p class="eyebrow">NEW FIELD NOTES · SEP 17</p><h2>Separate intelligence from experience. Learn AI by doing. Make every cable earn its place.</h2></div><a class="text-link" href="/feed.xml">Follow via RSS <span>→</span></a></div><div class="content-grid three">${card(essay)}${card(learningGuide)}${card(deskGuide)}</div></section>`, "<section class=\"guide-spotlight\">");

for (const [file, replacements] of Object.entries({
  "index.html": [[">41</dt><dd>Essays", ">42</dt><dd>Essays"], [">51</dt><dd>Field guides", ">53</dt><dd>Field guides"], ["View all 41 essays", "View all 42 essays"]],
  "essays.html": [[">41 / Transmissions", ">42 / Transmissions"], [">41</span> transmissions available", ">42</span> transmissions available"]],
  "field-guides.html": [[">51 / Field guides", ">53 / Field guides"], [">51</span> protocols available", ">53</span> protocols available"]],
})) {
...
upsert("essays/consciousness-is-the-weird-part.html", "DAILY 2026-09-17 AI CONSCIOUSNESS", `<div class="article-callout"><strong>Do not confuse performance with experience</strong><span>Read <a href="/essays/intelligence-is-not-consciousness.html">Intelligence Is Not Evidence of Consciousness</a> for a four-layer indicator and uncertainty framework.</span></div>`, "</article>");
upsert("guides/best-books-to-understand-ai.html", "DAILY 2026-09-17 AI ROADMAP", `<div class="article-callout"><strong>Turn the reading list into a curriculum</strong><span>Use <a href="/guides/how-to-learn-ai-without-chasing-every-update.html">the six-week AI learning roadmap</a> to connect concepts, experiments, evaluation and governance.</span></div>`, "</article>");
upsert("guides/usb-c-hub-vs-thunderbolt-dock-vs-monitor-dock.html", "DAILY 2026-09-17 DESK SWITCHING", `<div class="article-callout"><strong>Two computers change the wiring problem</strong><span>Compare a <a href="/guides/kvm-switch-vs-usb-switch-vs-dock.html">KVM switch, USB switch and dock</a> before adding another box to the desk.</span></div>`, "</article>");

let feed = readFileSync(join(root, "feed.xml"), "utf8").replace(/<lastBuildDate>[^<]+<\/lastBuildDate>/, `<lastBuildDate>${pubDate}</lastBuildDate>`).replace(/<!-- DAILY 2026-09-17 FEED START -->[\s\S]*?<!-- DAILY 2026-09-17 FEED END -->\n?/g, "");
...
const feedBlock = `<!-- DAILY 2026-09-17 FEED START -->\n${feedItems}\n<!-- DAILY 2026-09-17 FEED END -->\n`;
feed = feed.includes("<!-- DAILY 2026-09-16 FEED START -->") ? feed.replace("<!-- DAILY 2026-09-16 FEED START -->", `${feedBlock}    <!-- DAILY 2026-09-16 FEED START -->`) : feed.replace(/\s*<item>/, `\n${feedBlock}<item>`);
