...
import { pages } from "./content-2026-09-02.mjs";

const root = resolve(process.argv[2] || ".");
const siteUrl = "https://lifeinthesimulation.com";
const published = "2026-09-02";
const publishedHuman = "September 2, 2026";
const pubDate = "Wed, 02 Sep 2026 13:30:00 GMT";
...
const [essay, dnsGuide, phoneGuide] = pages;
...
upsert("essays.html", "DAILY 2026-09-02 ESSAY", `<section class="section section-pad"><div class="section-heading"><div><p class="eyebrow">LATEST TRANSMISSION</p><h2>A useful twin is a bounded model, not a second reality.</h2></div></div><div class="content-grid three">${card(essay)}</div></section>`);
upsert("field-guides.html", "DAILY 2026-09-02 GUIDES", `<section class="section section-pad"><div class="section-heading"><div><p class="eyebrow">LATEST FIELD GUIDES</p><h2>Move network trust deliberately. Make the phone earn its place.</h2></div></div><div class="content-grid three">${card(dnsGuide)}${card(phoneGuide)}</div></section>`);
upsert("index.html", "DAILY 2026-09-02 HOME", `<section class="latest-section section-pad"><div class="section-heading"><div><p class="eyebrow">NEW FIELD NOTES · SEP 2</p><h2>Test the twin. Choose the resolver. Put friction where it matters.</h2></div><a class="text-link" href="/feed.xml">Follow via RSS <span>→</span></a></div><div class="content-grid three">${card(essay)}${card(dnsGuide)}${card(phoneGuide)}</div></section>`, "<section class=\"guide-spotlight\">");

for (const [file, replacements] of Object.entries({
  "index.html": [[">29</dt><dd>Essays", ">30</dt><dd>Essays"], [">27</dt><dd>Field guides", ">29</dt><dd>Field guides"], ["View all 29 essays", "View all 30 essays"]],
  "essays.html": [[">29 / Transmissions", ">30 / Transmissions"], [">29</span> transmissions available", ">30</span> transmissions available"]],
  "field-guides.html": [[">27 / Field guides", ">29 / Field guides"], [">27</span> protocols available", ">29</span> protocols available"]],
...
upsert("essays/simulation-hypothesis-without-the-hype.html", "DAILY 2026-09-02 TWIN", `<div class="article-callout"><strong>A model is not a duplicate</strong><span>Use purpose, synchronization and uncertainty to evaluate the metaphor in <a href="/essays/digital-twin-is-not-a-second-reality.html">A Digital Twin Is Not a Second Reality</a>.</span></div>`, "<section class=\"article-endnote\">");
upsert("guides/private-web-browsers-firefox-brave-safari.html", "DAILY 2026-09-02 DNS", `<div class="article-callout"><strong>Choose the resolver layer</strong><span>Compare encryption, logging and filtering in <a href="/guides/private-dns-quad9-cloudflare-nextdns.html">Private DNS Without the Hype</a>.</span></div>`, "</article>");
upsert("guides/digital-environment-reset.html", "DAILY 2026-09-02 PHONE", `<div class="article-callout"><strong>Test hardware after configuration</strong><span>Compare dedicated minimalist phones with the locked-down device you already own in <a href="/guides/minimalist-phones-light-phone-punkt-locked-down-smartphone.html">Minimalist Phones Without the Hype</a>.</span></div>`, "</article>");

let feed = readFileSync(join(root, "feed.xml"), "utf8").replace(/<lastBuildDate>[^<]+<\/lastBuildDate>/, `<lastBuildDate>${pubDate}</lastBuildDate>`).replace(/<!-- DAILY 2026-09-02 FEED START -->[\s\S]*?<!-- DAILY 2026-09-02 FEED END -->\n?/g, "");
...
const feedBlock = `<!-- DAILY 2026-09-02 FEED START -->\n${feedItems}\n<!-- DAILY 2026-09-02 FEED END -->\n`;
feed = feed.includes("<!-- DAILY 2026-09-01 FEED START -->") ? feed.replace("<!-- DAILY 2026-09-01 FEED START -->", `${feedBlock}    <!-- DAILY 2026-09-01 FEED START -->`) : feed.replace(/\s*<item>/, `\n${feedBlock}<item>`);
...
sitemap = sitemap.replace(/\s*<!-- DAILY 2026-09-02 SITEMAP START -->[\s\S]*?<!-- DAILY 2026-09-02 SITEMAP END -->\s*/g, "");
...
sitemap = sitemap.replace("</urlset>", `\n<!-- DAILY 2026-09-02 SITEMAP START -->\n${urls}\n<!-- DAILY 2026-09-02 SITEMAP END -->\n</urlset>`);
writeFileSync(join(root, "sitemap.xml"), sitemap);

let llms = readFileSync(join(root, "llms.txt"), "utf8").replace(/\n?## Daily publication 2026-09-02[\s\S]*?(?=\n## |$)/g, "");
llms += `\n\n## Daily publication 2026-09-02\n${pages.map((page) => `- ${page.title}: ${siteUrl}/${page.path}`).join("\n")}\n`;
