...
const today = [
  "essays/attention-is-not-consciousness.html",
  "guides/how-to-write-a-personal-runbook.html",
  "guides/digital-photo-frames-local-cloud-smart-display.html"
];
const assets = [
  "/assets/visuals/attention-consciousness-editorial.webp",
  "/assets/visuals/personal-runbook-editorial.webp",
  "/assets/visuals/digital-photo-frames-editorial.webp"
];
...
for (const host of ["pubmed.ncbi.nlm.nih.gov", "plato.stanford.edu"]) if (!readFileSync(join(root, today[0]), "utf8").includes(host)) fail(`${today[0]}: missing authoritative ${host} source`);
for (const host of ["csrc.nist.gov", "wa.aws.amazon.com", "cisa.gov"]) if (!readFileSync(join(root, today[1]), "utf8").includes(host)) fail(`${today[1]}: missing authoritative ${host} source`);
for (const host of ["support.google.com", "support.apple.com", "ftc.gov"]) if (!readFileSync(join(root, today[2]), "utf8").includes(host)) fail(`${today[2]}: missing authoritative ${host} source`);
...
if (!/<lastBuildDate>Fri, 25 Sep 2026/.test(feed)) fail("feed: stale lastBuildDate");
...
for (const [hub, rel] of [["topics/simulation-theory.html", today[0]], ["topics/attention-agency.html", today[1]], ["topics/privacy-security.html", today[2]]]) if (!readFileSync(join(root, hub), "utf8").includes(`/${rel}`)) fail(`${hub}: missing ${rel}`);
for (const [source, target] of [["essays/consciousness-is-the-weird-part.html", today[0]], ["essays/intelligence-is-not-consciousness.html", today[0]], ["guides/personal-dependency-map.html", today[1]], ["essays/a-system-is-not-resilient-because-it-has-never-failed.html", today[1]], ["guides/personal-data-minimization.html", today[2]], ["guides/personal-digital-exit-plan.html", today[2]]]) if (!readFileSync(join(root, source), "utf8").includes(`/${target}`)) fail(`${source}: missing reciprocal link to ${target}`);
