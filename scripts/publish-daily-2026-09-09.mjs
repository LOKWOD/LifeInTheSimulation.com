...
import { pages } from "./content-2026-09-09.mjs";

const root = resolve(process.argv[2] || ".");
const siteUrl = "https://lifeinthesimulation.com";
const published = "2026-09-09";
const publishedHuman = "September 9, 2026";
const pubDate = "Wed, 09 Sep 2026 13:20:00 GMT";
...
const [essay, decisionGuide, shredderGuide] = pages;
...
upsert("essays.html", "DAILY 2026-09-09 ESSAY", `<section class="section section-pad"><div class="section-heading"><div><p class="eyebrow">LATEST TRANSMISSION</p><h2>A preference is evidence, not a whole person.</h2></div></div><div class="content-grid three">${card(essay)}</div></section>`);
upsert("field-guides.html", "DAILY 2026-09-09 GUIDES", `<section class="section section-pad"><div class="section-heading"><div><p class="eyebrow">LATEST FIELD GUIDES</p><h2>Preserve the decision. Destroy the paper.</h2></div></div><div class="content-grid three">${card(decisionGuide)}${card(shredderGuide)}</div></section>`);
upsert("index.html", "DAILY 2026-09-09 HOME", `<section class="latest-section section-pad"><div class="section-heading"><div><p class="eyebrow">NEW FIELD NOTES · SEP 9</p><h2>Separate preference from value. Preserve the decision. Destroy paper deliberately.</h2></div><a class="text-link" href="/feed.xml">Follow via RSS <span>→</span></a></div><div class="content-grid three">${card(essay)}${card(decisionGuide)}${card(shredderGuide)}</div></section>`, "<section class=\"guide-spotlight\">");

for (const [file, replacements] of Object.entries({
  "index.html": [[">35</dt><dd>Essays", ">36</dt><dd>Essays"], [">39</dt><dd>Field guides", ">41</dt><dd>Field guides"], ["View all 35 essays", "View all 36 essays"]],
  "essays.html": [[">35 / Transmissions", ">36 / Transmissions"], [">35</span> transmissions available", ">36</span> transmissions available"]],
  "field-guides.html": [[">39 / Field guides", ">41 / Field guides"], [">39</span> protocols available", ">41</span> protocols available"]],
})) {
...
upsert("essays/the-default-is-a-decision-someone-else-made.html", "DAILY 2026-09-09 PREFERENCE VALUE", `<div class="article-callout"><strong>A selected option is not a complete value statement</strong><span>Read <a href="/essays/preference-is-not-a-value.html">A Preference Is Not a Value</a> for a framework that separates observed behavior, inference and authority.</span></div>`, "</article>");
upsert("essays/automation-does-not-remove-responsibility.html", "DAILY 2026-09-09 AI DECISION RECORD", `<div class="article-callout"><strong>Preserve the human decision around the output</strong><span>Use <a href="/guides/how-to-write-an-ai-decision-record.html">How to Write an AI Decision Record</a> to document evidence, ownership, review and rollback.</span></div>`, "</article>");
upsert("guides/personal-data-minimization.html", "DAILY 2026-09-09 PAPER DESTRUCTION", `<div class="article-callout"><strong>Finish the retention plan</strong><span>Use <a href="/guides/paper-shredders-strip-cross-micro-cut.html">Paper Shredders Without the Hype</a> to match cut style, duty cycle or a service to the documents you actually need to destroy.</span></div>`, "</article>");

let feed = readFileSync(join(root, "feed.xml"), "utf8").replace(/<lastBuildDate>[^<]+<\/lastBuildDate>/, `<lastBuildDate>${pubDate}</lastBuildDate>`).replace(/<!-- DAILY 2026-09-09 FEED START -->[\s\S]*?<!-- DAILY 2026-09-09 FEED END -->\n?/g, "");
...
const feedBlock = `<!-- DAILY 2026-09-09 FEED START -->\n${feedItems}\n<!-- DAILY 2026-09-09 FEED END -->\n`;
feed = feed.includes("<!-- DAILY 2026-09-08 FEED START -->") ? feed.replace("<!-- DAILY 2026-09-08 FEED START -->", `${feedBlock}    <!-- DAILY 2026-09-08 FEED START -->`) : feed.replace(/\s*<item>/, `\n${feedBlock}<item>`);
