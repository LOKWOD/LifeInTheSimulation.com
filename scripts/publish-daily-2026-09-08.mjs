...
import { pages } from "./content-2026-09-08.mjs";

const root = resolve(process.argv[2] || ".");
const siteUrl = "https://lifeinthesimulation.com";
const published = "2026-09-08";
const publishedHuman = "September 8, 2026";
const pubDate = "Tue, 08 Sep 2026 13:13:00 GMT";
...
const [essay, experimentGuide, readerGuide] = pages;
...
upsert("essays.html", "DAILY 2026-09-08 ESSAY", `<section class="section section-pad"><div class="section-heading"><div><p class="eyebrow">LATEST TRANSMISSION</p><h2>An explanation still needs evidence.</h2></div></div><div class="content-grid three">${card(essay)}</div></section>`);
upsert("field-guides.html", "DAILY 2026-09-08 GUIDES", `<section class="section section-pad"><div class="section-heading"><div><p class="eyebrow">LATEST FIELD GUIDES</p><h2>Test the routine. Choose the ecosystem.</h2></div></div><div class="content-grid three">${card(experimentGuide)}${card(readerGuide)}</div></section>`);
upsert("index.html", "DAILY 2026-09-08 HOME", `<section class="latest-section section-pad"><div class="section-heading"><div><p class="eyebrow">NEW FIELD NOTES · SEP 8</p><h2>Inspect the reason. Test one variable. Buy the route into books.</h2></div><a class="text-link" href="/feed.xml">Follow via RSS <span>→</span></a></div><div class="content-grid three">${card(essay)}${card(experimentGuide)}${card(readerGuide)}</div></section>`, "<section class=\"guide-spotlight\">");

for (const [file, replacements] of Object.entries({
  "index.html": [[">34</dt><dd>Essays", ">35</dt><dd>Essays"], [">37</dt><dd>Field guides", ">39</dt><dd>Field guides"], ["View all 34 essays", "View all 35 essays"]],
  "essays.html": [[">34 / Transmissions", ">35 / Transmissions"], [">34</span> transmissions available", ">35</span> transmissions available"]],
  "field-guides.html": [[">37 / Field guides", ">39 / Field guides"], [">37</span> protocols available", ">39</span> protocols available"]],
})) {
...
upsert("essays/ai-accuracy-is-not-understanding.html", "DAILY 2026-09-08 AI EXPLANATIONS", `<div class="article-callout"><strong>A reason box needs its own test</strong><span>Read <a href="/essays/ai-explanation-is-not-the-mechanism.html">An AI Explanation Is Not the Mechanism</a> for the difference between plausible, useful and faithful explanations.</span></div>`, "</article>");
upsert("guides/thirty-day-attention-experiment.html", "DAILY 2026-09-08 ONE PERSON EXPERIMENT", `<div class="article-callout"><strong>Turn the reset into a comparison</strong><span>Use <a href="/guides/how-to-run-a-one-person-experiment.html">How to Run a One-Person Experiment</a> to define a baseline, one variable, a guardrail and a narrow conclusion.</span></div>`, "</article>");
upsert("guides/information-diet.html", "DAILY 2026-09-08 E READERS", `<div class="article-callout"><strong>Choose the route into books before the device</strong><span>Compare library access, file custody and distraction in <a href="/guides/e-readers-kindle-kobo-boox-library.html">E-Readers Without the Hype</a>.</span></div>`, "</article>");

let feed = readFileSync(join(root, "feed.xml"), "utf8").replace(/<lastBuildDate>[^<]+<\/lastBuildDate>/, `<lastBuildDate>${pubDate}</lastBuildDate>`).replace(/<!-- DAILY 2026-09-08 FEED START -->[\s\S]*?<!-- DAILY 2026-09-08 FEED END -->\n?/g, "");
...
const feedBlock = `<!-- DAILY 2026-09-08 FEED START -->\n${feedItems}\n<!-- DAILY 2026-09-08 FEED END -->\n`;
feed = feed.includes("<!-- DAILY 2026-09-07 FEED START -->") ? feed.replace("<!-- DAILY 2026-09-07 FEED START -->", `${feedBlock}    <!-- DAILY 2026-09-07 FEED START -->`) : feed.replace(/\s*<item>/, `\n${feedBlock}<item>`);
