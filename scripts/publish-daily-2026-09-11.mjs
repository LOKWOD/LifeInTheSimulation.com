...
import { pages } from "./content-2026-09-11.mjs";

const root = resolve(process.argv[2] || ".");
const siteUrl = "https://lifeinthesimulation.com";
const published = "2026-09-11";
const publishedHuman = "September 11, 2026";
const pubDate = "Fri, 11 Sep 2026 13:47:00 GMT";
...
const [essay, premortemGuide, upsGuide] = pages;
...
upsert("essays.html", "DAILY 2026-09-11 ESSAY", `<section class="section section-pad"><div class="section-heading"><div><p class="eyebrow">LATEST TRANSMISSION</p><h2>The rows are evidence. They are not the world.</h2></div></div><div class="content-grid three">${card(essay)}</div></section>`);
upsert("field-guides.html", "DAILY 2026-09-11 GUIDES", `<section class="section section-pad"><div class="section-heading"><div><p class="eyebrow">LATEST FIELD GUIDES</p><h2>Imagine the failure. Protect the useful load.</h2></div></div><div class="content-grid three">${card(premortemGuide)}${card(upsGuide)}</div></section>`);
upsert("index.html", "DAILY 2026-09-11 HOME", `<section class="latest-section section-pad"><div class="section-heading"><div><p class="eyebrow">NEW FIELD NOTES · SEP 11</p><h2>Audit the dataset. Run the pre-mortem. Size the power bridge.</h2></div><a class="text-link" href="/feed.xml">Follow via RSS <span>→</span></a></div><div class="content-grid three">${card(essay)}${card(premortemGuide)}${card(upsGuide)}</div></section>`, "<section class=\"guide-spotlight\">");

for (const [file, replacements] of Object.entries({
  "index.html": [[">37</dt><dd>Essays", ">38</dt><dd>Essays"], [">43</dt><dd>Field guides", ">45</dt><dd>Field guides"], ["View all 37 essays", "View all 38 essays"]],
  "essays.html": [[">37 / Transmissions", ">38 / Transmissions"], [">37</span> transmissions available", ">38</span> transmissions available"]],
  "field-guides.html": [[">43 / Field guides", ">45 / Field guides"], [">43</span> protocols available", ">45</span> protocols available"]],
})) {
...
upsert("essays/ai-benchmark-is-not-the-real-world.html", "DAILY 2026-09-11 DATASET", `<div class="article-callout"><strong>Inspect what became test data</strong><span>Read <a href="/essays/dataset-is-not-the-world.html">A Dataset Is Not the World</a> to expose collection, labels, omissions and time before trusting the score.</span></div>`, "</article>");
upsert("guides/reversibility-test-for-decisions.html", "DAILY 2026-09-11 PREMORTEM", `<div class="article-callout"><strong>Give hard-to-reverse plans a failure rehearsal</strong><span>Use <a href="/guides/how-to-run-a-premortem.html">the pre-mortem protocol</a> to convert plausible failure paths into owners, triggers and controls.</span></div>`, "</article>");
upsert("guides/home-wifi-router-vs-mesh-vs-access-points.html", "DAILY 2026-09-11 UPS", `<div class="article-callout"><strong>Keep the network alive only when the job requires it</strong><span>Use <a href="/guides/ups-battery-backup-standby-line-interactive-online.html">UPS Battery Backups Without the Hype</a> to size a measured power bridge for selected network and computer loads.</span></div>`, "</article>");

let feed = readFileSync(join(root, "feed.xml"), "utf8").replace(/<lastBuildDate>[^<]+<\/lastBuildDate>/, `<lastBuildDate>${pubDate}</lastBuildDate>`).replace(/<!-- DAILY 2026-09-11 FEED START -->[\s\S]*?<!-- DAILY 2026-09-11 FEED END -->\n?/g, "");
...
const feedBlock = `<!-- DAILY 2026-09-11 FEED START -->\n${feedItems}\n<!-- DAILY 2026-09-11 FEED END -->\n`;
feed = feed.includes("<!-- DAILY 2026-09-10 FEED START -->") ? feed.replace("<!-- DAILY 2026-09-10 FEED START -->", `${feedBlock}    <!-- DAILY 2026-09-10 FEED START -->`) : feed.replace(/\s*<item>/, `\n${feedBlock}<item>`);
