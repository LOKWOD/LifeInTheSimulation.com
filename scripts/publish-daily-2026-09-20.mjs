...
import { pages } from "./content-2026-09-20.mjs";

const root = resolve(process.argv[2] || ".");
const siteUrl = "https://lifeinthesimulation.com";
const published = "2026-09-20";
const publishedHuman = "September 20, 2026";
const pubDate = "Sun, 20 Sep 2026 13:28:00 GMT";
...
const [essay, notificationGuide, recorderGuide] = pages;
...
upsert("essays.html", "DAILY 2026-09-20 ESSAY", `<section class="section section-pad"><div class="section-heading"><div><p class="eyebrow">LATEST TRANSMISSION</p><h2>Count observations, not the links that repeat them.</h2></div></div><div class="content-grid three">${card(essay)}</div></section>`);
upsert("field-guides.html", "DAILY 2026-09-20 GUIDES", `<section class="section section-pad"><div class="section-heading"><div><p class="eyebrow">LATEST FIELD GUIDES</p><h2>Route the interruption. Choose the recorder by the room.</h2></div></div><div class="content-grid three">${card(notificationGuide)}${card(recorderGuide)}</div></section>`);
upsert("index.html", "DAILY 2026-09-20 HOME", `<section class="latest-section section-pad"><div class="section-heading"><div><p class="eyebrow">NEW FIELD NOTES · SEP 20</p><h2>Trace the source. Control the interruption. Capture the right signal.</h2></div><a class="text-link" href="/feed.xml">Follow via RSS <span>→</span></a></div><div class="content-grid three">${card(essay)}${card(notificationGuide)}${card(recorderGuide)}</div></section>`, "<section class=\"guide-spotlight\">");

for (const [file, replacements] of Object.entries({
  "index.html": [[">44</dt><dd>Essays", ">45</dt><dd>Essays"], [">57</dt><dd>Field guides", ">59</dt><dd>Field guides"], ["View all 44 essays", "View all 45 essays"]],
  "essays.html": [[">44 / Transmissions", ">45 / Transmissions"], [">44</span> transmissions available", ">45</span> transmissions available"]],
  "field-guides.html": [[">57 / Field guides", ">59 / Field guides"], [">57</span> protocols available", ">59</span> protocols available"]],
})) {
...
upsert("guides/how-to-read-a-scientific-paper.html", "DAILY 2026-09-20 CITATION CASCADE", `<div class="article-callout"><strong>Ten references can still descend from one observation</strong><span>Use the <a href="/essays/ten-links-can-still-be-one-source.html">source-provenance map</a> to separate repetition, synthesis and independent evidence.</span></div>`, "</article>");
upsert("guides/how-to-verify-an-ai-citation.html", "DAILY 2026-09-20 CITATION CASCADE", `<div class="article-callout"><strong>Verification continues after the source exists</strong><span>Trace whether <a href="/essays/ten-links-can-still-be-one-source.html">many citations share one evidential ancestor</a> before treating them as corroboration.</span></div>`, "</article>");
upsert("guides/how-to-separate-claim-evidence-inference.html", "DAILY 2026-09-20 CITATION CASCADE", `<div class="article-callout"><strong>Map the evidence family</strong><span>The <a href="/essays/ten-links-can-still-be-one-source.html">citation-cascade guide</a> shows why multiple documents may still contain only one underlying observation.</span></div>`, "</article>");
upsert("guides/attention-reset.html", "DAILY 2026-09-20 NOTIFICATION LADDER", `<div class="article-callout"><strong>Turn silence into a routing policy</strong><span>Build a <a href="/guides/notification-escalation-ladder.html">notification escalation ladder</a> that preserves emergency paths while batching everything that can wait.</span></div>`, "</article>");
upsert("guides/website-blockers-for-focus.html", "DAILY 2026-09-20 NOTIFICATION LADDER", `<div class="article-callout"><strong>Control arrival before controlling access</strong><span>Use the <a href="/guides/notification-escalation-ladder.html">three-tier interruption system</a> before adding stricter blocking friction.</span></div>`, "</article>");
upsert("guides/private-note-taking-apps.html", "DAILY 2026-09-20 VOICE RECORDERS", `<div class="article-callout"><strong>Audio capture needs the same exit discipline</strong><span>Compare a <a href="/guides/voice-recorders-dedicated-phone-usb-microphone.html">dedicated recorder, phone app and USB microphone</a> by privacy, export and failure mode.</span></div>`, "</article>");
upsert("guides/personal-digital-exit-plan.html", "DAILY 2026-09-20 VOICE RECORDERS", `<div class="article-callout"><strong>Do not let recordings become service hostages</strong><span>The <a href="/guides/voice-recorders-dedicated-phone-usb-microphone.html">voice-recorder decision guide</a> includes raw-file export, backup and transcription boundaries.</span></div>`, "</article>");

let feed = readFileSync(join(root, "feed.xml"), "utf8").replace(/<lastBuildDate>[^<]+<\/lastBuildDate>/, `<lastBuildDate>${pubDate}</lastBuildDate>`).replace(/<!-- DAILY 2026-09-20 FEED START -->[\s\S]*?<!-- DAILY 2026-09-20 FEED END -->\n?/g, "");
...
const feedBlock = `<!-- DAILY 2026-09-20 FEED START -->\n${feedItems}\n<!-- DAILY 2026-09-20 FEED END -->\n`;
feed = feed.includes("<!-- DAILY 2026-09-19 FEED START -->") ? feed.replace("<!-- DAILY 2026-09-19 FEED START -->", `${feedBlock}    <!-- DAILY 2026-09-19 FEED START -->`) : feed.replace(/\s*<item>/, `\n${feedBlock}<item>`);
