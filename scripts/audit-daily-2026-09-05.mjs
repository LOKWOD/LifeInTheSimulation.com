...
const today = [
  "essays/ai-benchmark-is-not-the-real-world.html",
  "guides/how-to-verify-an-ai-citation.html",
  "guides/external-ssd-vs-hard-drive-vs-nas-backup.html"
];
const assets = [
  "/assets/visuals/ai-benchmark-editorial.webp",
  "/assets/visuals/verify-ai-citation-editorial.webp",
  "/assets/visuals/backup-storage-editorial.webp"
];
...
if (!/>33<\/span> transmissions available/.test(essaysArchive)) fail("essays archive: stale result count");
if (!/>35<\/span> protocols available/.test(guidesArchive)) fail("field guides archive: stale result count");
if (!/>33<\/dt><dd>Essays/.test(home) || !/>35<\/dt><dd>Field guides/.test(home)) fail("homepage: stale library counts");
...
const archiveCoverage = [
  ["essays", essaysArchive, 33],
  ["guides", guidesArchive, 35],
];
...
for (const host of ["nist.gov", "crfm.stanford.edu"]) if (!readFileSync(join(root, today[0]), "utf8").includes(host)) fail(`${today[0]}: missing authoritative ${host} source`);
for (const host of ["crossref.org", "ncbi.nlm.nih.gov"]) if (!readFileSync(join(root, today[1]), "utf8").includes(host)) fail(`${today[1]}: missing authoritative ${host} source`);
for (const host of ["nist.gov", "cisa.gov", "news.samsung.com", "support.apple.com", "support.microsoft.com"]) if (!readFileSync(join(root, today[2]), "utf8").includes(host)) fail(`${today[2]}: missing official ${host} source`);
if (!/Commercial disclosure[\s\S]*They are not affiliate links/i.test(readFileSync(join(root, today[2]), "utf8"))) fail(`${today[2]}: missing clear non-affiliate commercial disclosure`);

if (!/<lastBuildDate>Sat, 05 Sep 2026/.test(feed)) fail("feed: stale lastBuildDate");
